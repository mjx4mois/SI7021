...
---------------------------------------------------------------------- */
#ifndef __SI7021_FUNCTION__
#define __SI7021_FUNCTION__
...
CHAR8S SI7021_READ_TEMPERATURE(FLOAT *t,CHAR8U mode)
{
		...
		//check mode 
		if(mode !=SI7021_MODE0 && mode !=SI7021_MODE1  && mode !=SI7021_MODE2 && mode !=SI7021_MODE3 )return -1; //error mode value
...
CHAR8S SI7021_CHECK_BATTERY_STATUS(CHAR8U *status)
{
		...
		if(status_return!=0) return -1;

		//check bit 6
		if(data & 0x40) *status = 0x01;	//   [1.8<VDD<1.9v]   VDD not good!
	       else   *status = 0x00; 	//    [VDD>1.9v] VDD OK!
...
CHAR8S SI7021_CRC_CHECKSUM(CHAR8U *data, CHAR8U bytes,CHAR8U checksum)
{
  	 CHAR8U crc=0,cnt,cnt2; //cnt2 counting variable    
	    ...
	    if(data[0]== 0 && data[1]== 0 && checksum==0) return -4 ;//read data not good!  
...
#endif		//#ifndef __SI7021_FUNCTION__
